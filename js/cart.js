...
if (btnCategorias && categorias) {
  ...
  document.addEventListener('click', (e) => {
    if (categorias.classList.contains('active')) {
      ...
      if (!categoriasContenedor.contains(e.target) && e.target !== btnCategorias && !e.target.closest('.product-page-back-container .btnCategorias') && !isEditorClick) {
        window.closePixisMenu();
      }
    }
  });
...
(function () {
  ...
  // ── SWIPE BACK GESTURE en página de producto ──
  // Detecta un swipe horizontal hacia la derecha para cerrar el modal del producto.
  (function initSwipeBack() {
    ...
    document.addEventListener('touchstart', function(e) {
      if (!document.body.classList.contains('product-page-active')) return;
      // Si la imagen tiene zoom activo, no cerrar por swipe
    ...
    document.addEventListener('touchmove', function(e) {
      ...
      // Si el movimiento es más vertical que horizontal, cancelar trackeo
      if (dY > 25 && dY > Math.abs(dX)) {
        sbTracking = false;
      }
    }, { passive: true });
    ...
    document.addEventListener('touchend', function(e) {
      ...
      // Swipe derecha >= 60px con movimiento claramente horizontal (Y < 45% de X)
